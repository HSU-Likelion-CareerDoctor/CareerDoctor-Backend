...
public class SpecCertificateServiceImpl implements SpecCertificateService {
    ...
    public SpecCertificate createSpecCertificate(User user, Degree degree, List<Language> languages,
                                                 List<Certificate> certificates, List<Activity> activities, List<Career> careers,
                                                 List<Etc> etcs, String name, String birth, Gender gender, Level level, boolean isPublic) {

        SpecCertificate specCertificate = SpecCertificate.builder()
                .user(user)
                .degree(degree)
                .languages(languages)
                ...
                .isPublic(isPublic)
                .build();
        return specRepository.save(specCertificate);
    }
    ...
    public ResponseEntity<CustomApiResponse<?>> sertificateSpec(CertificateSpecDto certificateSpecDto) {
        ...
        if(certificateSpecDto.getUserId() == null) {
            ...
            LanguageName languageName = certificateSpecDto.getLanguageName();
            LanguageLevel languageLevel = certificateSpecDto.getLanguageLevel();
            //List<Language> language = new Language(certificateSpecDto.getLanguageName(), certificateSpecDto.getLanguageLevel()); // Language 자체가 하나의 entity? // 따로 저장 후 연결?
            // 보유 자격증, 대외활동, 경력사항도 똑같ㅇㅣ..

            Language language = Language.builder()
                    .languageName(certificateSpecDto.getLanguageName())
                    .languageLevel(certificateSpecDto.getLanguageLevel())
                    .build();
